...
function cargarCanto(partitura) {
    ...
    cargarSeccion('asamblea', partitura.asamblea, partitura.asambleaAcordes);
    cargarSeccion('cantor', partitura.cantor, partitura.cantorAcordes);

    acordesOriginales = extraerAcordesOriginales(partitura); // Guardar acordes originales
    configurarSelectores(); // Configurar la interactividad de los selectores
}

function cargarSeccion(seccionId, letras, acordes) {
    ...
    letras.forEach((letra, index) => {
        ...
        if (acordes && acordes[index]) {
            acordes[index].forEach(acorde => {
                html += `<select class="acorde-select" style="left: ${acorde.posicion}px;"
    ...
    acordeSelectores.forEach(selector => {
        selector.addEventListener('change', function() {
            ...
            acordesEnLinea.forEach(otroSelector => {
                ...
                otroSelector.dataset.original = otroSelector.value; // Actualizar el original
            });
        });
    });
}
